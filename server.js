...
app.use((req, res, next) => {
  ...
  if (blockedIps.includes(userIp)) {
      ...
      return res.status(403).send('You have been IP banned for suspisous activity, if you think this is an issue reach us at: appeal@botbridge.net'); // Return a 403 Forbidden response
  }
...
const MAX_ACTIVE_USERS = 120; // Max active users
...
const options = {
    key: fs.readFileSync('certs/private2.key.pem'),
    cert: fs.readFileSync('certs/domain2.cert.pem'),
};
