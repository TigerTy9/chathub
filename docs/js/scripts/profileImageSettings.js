...
async function fetchProfileImage(username) {
    try {
        ...
        const response = await fetch(`https://characters.botbridge.net/api/profile-picture/${username}`);
        
        if (response.ok) {
            // The server will return the image file directly, so we set it as the image src
            const imageUrl = `https://characters.botbridge.net/api/profile-picture/${username}`;
...
uploadImageButton.addEventListener('click', async (event) => {
    ...
    if (file) {
        try {
            ...
            const response = await fetch('https://characters.botbridge.net/api/upload/profile-picture', {
                method: 'POST',
                headers: {
